...
public class ClientQuadTreeNode {
	private ClientQuadTreeNode northWestNode;
	private ClientQuadTreeNode northEastNode;
	private ClientQuadTreeNode southWestNode;
	private ClientQuadTreeNode southEastNode;
	private QuadTreeNodeBox nodeBox;
	public final String nodeID;
	public final int depth;
	public static final int maxDepth = 30;

	public ClientQuadTreeNode(QuadTreeNodeBox nodeBox, String nodeID, int depth){
	...
	private ClientQuadTreeNode subdivide(double x, double y){
		...
		if(y > yMid){
			if(x < xMid){
				if(northWestNode == null){
					...
					northWestNode = new ClientQuadTreeNode(northWestBox, nodeID+"0", depth+1);
				}
			...
			if(northEastNode == null){
				...
				northEastNode = new ClientQuadTreeNode(northEastBox, nodeID+"1", depth+1);
			}
		...
		if(x < xMid){
			if(southWestNode == null){
				...
				southWestNode = new ClientQuadTreeNode(southWestBox, nodeID+"2", depth+1);
			}
			return (ClientQuadTreeNode) southWestNode;
		}
		if(southEastNode == null){
			QuadTreeNodeBox southEastBox = new QuadTreeNodeBox(xMid, nodeBox.y0, nodeBox.x1, yMid);
			southEastNode = new ClientQuadTreeNode(southEastBox, nodeID+"3", depth+1);
		}
	...
	public String geoPointToQuad(LatLng point, int depth){
		...
		if(this.depth < maxDepth){
			ClientQuadTreeNode currentNode = subdivide(point.latitude, point.longitude);
