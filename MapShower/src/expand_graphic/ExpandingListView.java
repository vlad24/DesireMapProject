...
import com.example.mapshower.R;
...
public class ExpandingListView extends ListView {
	 ...
	 */
	private void getTopAndBottomTranslations(int top, int bottom, int yDelta,
			boolean isExpanding) {
		int yTranslateTop = 0;
		...
		mTranslate[0] = yTranslateTop;
		mTranslate[1] = yTranslateBottom;
	//	return new int[] {yTranslateTop, yTranslateBottom}; optimize
	}
	...
	//	final ExpandableListItem viewObject = (ExpandableListItem) getItemAtPosition(getPositionForView(view));
		...
		//final HashMap<View, int[]> oldCoordinates = new HashMap<View, int[]>(); optimize
		...
		final View expandingLayout = holder.expandingView; //view.findViewById(R.id.expanding_layout); optimize
		...
		observer.addOnPreDrawListener(expandPreDrawListener);
...
//				(getPositionForView(view)); //optimize
		...
		view.setLayoutParams(collapseLayoutParams); //optimize

		/* Add an onPreDraw listener. */
		observer = getViewTreeObserver();
		observer.addOnPreDrawListener(collapsePreDrawListener);
